...
static int communicate_in_x(const sdecomp_info_t * sdecomp_info, const sdecomp_pencil_t pencil, const size_t n_add, const size_t mysizes_tmp[3], double * array_tmp){
...
static int communicate_in_y(const sdecomp_info_t * sdecomp_info, const sdecomp_pencil_t pencil, const size_t n_add, const size_t mysizes_tmp[3], double * array_tmp){
  MPI_Comm comm_cart = MPI_COMM_NULL;
  sdecomp.get_comm_cart(sdecomp_info, &comm_cart);
  // check negative / positive neighbour ranks
  int neighbours[2] = {MPI_PROC_NULL, MPI_PROC_NULL};
...
static int communicate_in_z(const sdecomp_info_t * sdecomp_info, const sdecomp_pencil_t pencil, const size_t n_add, const size_t mysizes_tmp[3], double * array_tmp){
  MPI_Comm comm_cart = MPI_COMM_NULL;
  sdecomp.get_comm_cart(sdecomp_info, &comm_cart);
  // check negative / positive neighbour ranks
  int neighbours[2] = {MPI_PROC_NULL, MPI_PROC_NULL};
...
int contour3d_contour_extend_domain(
  ...
  size_t mysizes[3] = {0};
  size_t offsets[3] = {0};
  for(sdecomp_dir_t dir = 0; dir < 3; dir++){
  ...
  size_t mysizes_tmp[3] = {0};
  size_t offsets_tmp[3] = {0};
  for(sdecomp_dir_t dir = 0; dir < 3; dir++){
    mysizes_tmp[dir] = mysizes[dir] + 2 * n_add;
    offsets_tmp[dir] = offsets[dir];
  }
  // allocate the extended array and pack the original array
  double * array_tmp = contour3d_calloc(mysizes_tmp[0] * mysizes_tmp[1] * mysizes_tmp[2], sizeof(double));
  if(NULL == array_tmp){
    return 1;
  }
  for(size_t k = 0; k < mysizes[2]; k++){
    for(size_t j = 0; j < mysizes[1]; j++){
      for(size_t i = 0; i < mysizes[0]; i++){
        ...
        array_tmp[index_tmp] = contour_obj->array[index];
      }
    }
  }
  ...
  //   for each dimension(x-y-z: 3), for each direction(neg-pos: 2)
  bool clip[3][2] = {
    {false, false},
    {false, false},
    {false, false},
  ...
  *array_ext = contour3d_calloc(mysizes_ext[0] * mysizes_ext[1] * mysizes_ext[2], sizeof(double));
  if(NULL == *array_ext){
    return 1;
  }
